from commands.pdf.helpers import download_pdf, upload_pdf
import os

def register_pdf_parser(subparsers):
   # TODO: incomplete pdf parser
   pdf_parser = subparsers.add_parser('pdf', help='Used to download and upload pdfs')
   pdf_parser.add_argument('--roll', metavar='ROLL', help='gets the pdf of the specified roll')
   ...
   pdf_download_subparser.add_argument('url', help='url from where pdfs are to be downloaded')
   pdf_download_subparser.add_argument('semester', help='specifies the [semester] for pdf fetch')
   ...
   pdf_upload_subparser.add_argument('url', help='http url to save the pdf')
